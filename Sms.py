...
def load():#создаем функцию загрузки
    try:#пробуем открыть файл
        with open('save.txt', 'r') as file:#открываем файл для чтения,
            ...
            sender_email_entry.insert(0, info[0])#вставляем информацию в поле,
            # на нулевое место в самое начало строки вставляем что-то из нашей информации.
            # первый элемент с индексом ноль.это поле отправителя
            recipient_email_entry.insert(0, info[1])#вставляем информацию в поле получателя,
            # во вторую строку с индексом 1
            password_entry.insert(0, info[2])#вставляем информацию в поле пароля,
